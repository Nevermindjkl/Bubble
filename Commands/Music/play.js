...
exports.run = async (Client, message, args, QueueOptions) => {
   ...
   if(Premium == false){
    ...
    var TempURL = await ytdl.validateURL(args[1])
    if (!TempURL) return message.channel.send(InactiveURL)

    var URLInfo = await ytdl.getInfo(args[1]);
    ...
    if (!QueueData.dispatcher) {
        ...
        .setAuthor(URLInfo.videoDetails.title, message.author.displayAvatarURL(), args[1])
        .setColor(0xFF8C00)
...
async function Play(Client, QueueOptions, QueueData) {
    ...
    var ActiveRequest = new Discord.MessageEmbed()
    .setAuthor(QueueData.Queue[0].Title, QueueData.Queue[0].RequesterDefualt.displayAvatarURL(), QueueData.Queue[0].URL)
    .setColor(0xFF8C00)
